...
{
	private int MIME_TYPE,MIME_TYPE_INTERNAL;
    private GridView DeviceImageVideoGridview, SelectedElementGridview;
    private LinearLayout SelectedElemenetContainer, controlHeader;
    ...
    private LinearLayout imageVideoContainer, mainContainer;
    public float i = 1;
    ...
    private boolean imagePreviewScaleTyle_isMatrix = true;
    public int DeviceImageVideoGridview_destination_height;
    public loadbackgroundPictureTask task;
	private FakeR fakeR;
	@Override
	protected void onCreate(Bundle savedInstanceState) {
		...
		controlHeader = (LinearLayout) findViewById(fakeR.getId("id", "controlHeader"));
        DeviceImageVideoGridview = (GridView) findViewById(fakeR.getId("id", "gridView"));
        SelectedElemenetContainer = (LinearLayout) findViewById(fakeR.getId("id", "RelativeLayout1"));
        ...
        mainContainer = (LinearLayout) findViewById(fakeR.getId("id", "container"));
        ...
        DeviceImageVideoGridview_Adapter = new DeviceImageVideoGridviewAdapter(this, fakeR.getId("layout", "simple_list_item_1"), data, Glide.with(this),this);
   ...
   if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN_MR1) {
        ...
        DeviceImageVideoGridview.setOnItemClickListener(new OnItemClickListener() {
            public void onItemClick(AdapterView<?> parent, View v, int position, long id) {
                ...
                if (item.isVideo()) {
                    ...
                    source.setText(getResources().getString(fakeR.getId("string", "from")) + " " + item.getTitle());
                    ...
                    imagePreviewScaleTyle_isMatrix = true;
                    if (task != null)
                        task.cancel(true);

                    task = new loadbackgroundPictureTask(imageVideoContainer, item.getUrl(), getBaseContext(),true);
                    task.execute();
    ...
    {
        final ObjectAnimator anim2 = ObjectAnimator.ofInt(DeviceImageVideoGridview, "height", DeviceImageVideoGridview.getHeight(), ((screenheight - (140 + 100)) / 2));
        ...
        anim.addListener(new Animator.AnimatorListener() {
            public void onAnimationCancel(final Animator animation) {}
            public void onAnimationRepeat(final Animator animation) {}
            public void onAnimationStart(final Animator animation) {anim2.start();}
            @Override
            public void onAnimationEnd(final Animator animation) {
                showed = true;
            }
...
if (videoCursor.getColumnCount() > 0)
{
...
if (imageCursor.getColumnCount() > 0)
{
    ...
    /*
        ***********************************************************************************************************************************************
        ***********************************************************************************************************************************************
                            display the image of video   to  effect directly to our listview without reload all elements
        ***********************************************************************************************************************************************
        ***********************************************************************************************************************************************
         */
    //public void updateVideoInListview(Bitmap image,ImageOrVideoItem video) {
    public void updateVideoInListview() {
            DeviceImageVideoGridview_Adapter.notifyDataSetChanged();

       // if(DeviceImageVideoGridview_Adapter!=null)
     /*  if(false)
        {   
                    View v = DeviceImageVideoGridview.getChildAt(data.indexOf(video) -
                        DeviceImageVideoGridview.getFirstVisiblePosition());
                if (v == null)
                    return;
                ImageView picture = (ImageView) v.findViewById(fakeR.getId("id", "imageS"));
    ...
    private void selectPictureVideo(int index) {
        ...
        if (DeviceImageVideoGridview_Adapter.isMultiSelectionActiv()) {
            if (data.get(index).isMultiSelectionState())
            {
                ...
                //selectionbox.setImageResource(fakeR.getId("drawable", "selected_not"));
            ...
            else
            {
                if (!(SelectedElement_List.size() == limit_Select))
                {
                    ...
                    //selectionbox.setImageResource(fakeR.getId("drawable", "selected"));
